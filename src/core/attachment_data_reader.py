...
class AttachmentDataReader:
    ...
    def load_attachment_data(self):
        ...
        try:
            ...
            for sheet_name in scid_sheets:
                ...
                scid = Utils.normalize_scid(scid)
    ...
    def get_scid_data(self, scid):
        ...
        normalized_scid = Utils.normalize_scid(scid)
        ...
        try:
            ...
            for provider, keywords in telecom_keywords.items():
                ...
                if not provider_rows.empty:
                    provider_rows['height_numeric'] = pd.to_numeric(
                        provider_rows['height_in_inches'].astype(str).str.replace('"', '').str.replace('″', ''),
                        errors='coerce'
                    )
                    provider_rows = provider_rows.dropna(subset=['height_numeric'])
                    
                    if not provider_rows.empty:
                        provider_rows = provider_rows.sort_values(by='height_numeric')
                        
                        heights = [
                            Utils.inches_to_feet_format(str(int(row['height_numeric'])))
                            for _, row in provider_rows.iterrows()
                        ]
                        combined_heights = ', '.join(heights)
                        decimal_values = [
                            float(row['height_numeric']) / 12
                            for _, row in provider_rows.iterrows()
                        ]
                        ...
                        attachments[provider] = {
                            ...
                            'company': provider_rows.iloc[0]['company'],
                            'measured': provider_rows.iloc[0]['measured']
                        }
            return attachments
        except Exception as e:
